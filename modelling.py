...
hyperparameter_dict = {SGDRegressor(): {'learning_rate': ['constant', 'optimal', 'invscaling'], 'loss': ['squared_error', 'huber', 'epsilon_insensitive'], 'penalty': ['l1', 'l2', 'elasticnet'], 'alpha': [0.0001, 0.001, 0.01, 0.1, 1], 'max_iter': [5000, 8000, 10000, 15000, 20000]}, DecisionTreeRegressor(): {'splitter': ['best', 'random'], 'min_samples_split': [5, 10, 15, 20, 30], 'min_samples_leaf': [5, 10, 15, 20], 'max_leaf_nodes': [None, 20, 40, 60, 80, 100]}, RandomForestRegressor(): {'n_estimators': [50, 100, 200, 300], 'min_samples_split': [5, 10, 15, 20, 30], 'min_samples_leaf': [5, 10, 15, 20], 'max_leaf_nodes': [None, 20, 40, 60, 80, 100]}, GradientBoostingRegressor(): {'loss': ['squared_error', 'absolute_error', 'huber', 'quantile'], 'min_samples_split': [5, 10, 15, 20, 30], 'min_samples_leaf': [5, 10, 15, 20], 'max_leaf_nodes': [None, 20, 40, 60, 80, 100]}}
...
def custom_tune_regression_model_hyperparameters(hyperparameters, model = models, X_train = X_train, y_train = y_train, X_test = X_test, y_test = y_test, X_val = X_val, y_val = y_val):
    for k, v in hyperparameters.items():
        for combination in product(*v.values()):
            parameters = dict(zip(v, combination)) 
        best_score = -100000
        for l in range( len(parameters)):
            model = k.set_params(**parameters).fit(X_train, y_train)
            print(model)
            print(type(model))
        ...
        #print('Best score for', k, 'is: ', best_score)
...
#         gridsearch = GridSearchCV(m, param_grid)
#         gridsearch = gridsearch.fit(X_train, y_train)
#         # best_params = gridsearch.best_params_
#         # accuracy = gridsearch.best_score_
#         # print(m, best_params, accuracy)
